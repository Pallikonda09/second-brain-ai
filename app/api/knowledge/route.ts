...
import { summarizeKnowledge, generateTags } from "@/lib/groq";

// GET /api/knowledge?search=&page=1&limit=10
...
export async function POST(req: NextRequest) {
  ...
  // Run AI summarization + tagging in parallel
  const [summary, tags] = await Promise.all([
    summarizeKnowledge(content),
    generateTags(`${title}\n\n${content}`),
  ]);
